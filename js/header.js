...
window.addEventListener('scroll', function() {
    ...
    // Verifica se header e logo existem antes de acessá-los
    if (!header || !logo) {
      return;
    }
...
// Função global para logout (será chamada pelo onclick)
window.performLogout = function(event) {
    event.preventDefault(); // Impede o link de navegar para '#'
    sessionStorage.removeItem('welcomeUserName'); // Limpa o nome do usuário
    window.location.href = '/backend/logout'; // Redireciona para a rota de logout
}
...
function ativarMenuLateral() {
    ...
    if (btnMore && menuLateral && botaoFechar && overlay) {
        ...
        console.warn("Algum dos elementos do menu lateral não foi encontrado no DOM ao tentar ativar listeners.");
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const userName = sessionStorage.getItem('welcomeUserName'); // Verifica se há usuário logado

    if (navElement) { // Garante que o container do navbar existe

        // --- Define os itens principais da navegação desktop (SEM A SAUDAÇÃO) ---
        ...
        // --- FIM DOS ITENS DESKTOP ---

        // --- Define o item de autenticação para o menu lateral (Com saudação discreta) ---
        let sideMenuAuthItemHTML = '';
        if (userName) {
            // Se logado, mostra saudação, "Minha Conta" e "Sair"
            ...
            // Se não logado, mostra "Painel (Login)" que abre o modal
            sideMenuAuthItemHTML = '<li><a href="#" onclick="abrirLoginModal(); const menu = document.getElementById(\'menu-lateral\'); if(menu) menu.classList.remove(\'aberto\'); const overlay = document.getElementById(\'fundo-escuro\'); if(overlay) overlay.classList.remove(\'ativo\'); return false;">Painel (Login)</a></li>';
        }
            ...
            <nav class="menu">
                ...
                <ul class="menu-navegacao">
                 ...
                 <button id="btn-more" aria-label="Mais opções">
                    ...
                    <img src="img/icons/mais.svg" alt="mais" class="more">
                    </div>
                </button>
                </div>
            </nav>
